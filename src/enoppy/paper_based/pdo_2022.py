...
# --------------------------------------------------%
"""Engineering problems from the PDO 2022 paper."""
...
class WeldedBeamProblem(Engineer):
    ...
    def __init__(self, f_penalty=None):
        ...
        self._n_objs = 1
        self._n_cons = 11
        self._bounds = [(0.1, 2.0), (0.1, 10.0), (0.1, 10.0), (0.1, 2.0)]
    ...
    def get_cons(self, x):
        ...
        Pc_X = 4.013 * self.E * np.sqrt(x[2] ** 2 * x[3] ** 6 / 36) / self.L**2 * (1.0 - x[2] * np.sqrt(self.E / (4 * self.G)) / (2 * self.L))
...
class PressureVesselProblem(Engineer):
    ...
    def __init__(self, f_penalty=None):
        ...
        self._n_objs = 1
        self._n_cons = 4
        self._bounds = [(0.0, 99.0), (0.0, 99.0), (10.0, 200.0), (10.0, 200.0)]
        self.check_penalty_func(f_penalty)
    ...
    def get_cons(self, x):
        ...
        g1 = -x[0] + 0.0193 * x[2]
        g2 = -x[2] + 0.00954 * x[2]
        g3 = -np.pi * x[1] ** 2 * x[3] - 4.0 / 3 * np.pi * x[2] ** 3 + 750 * 1728
        g4 = -240 + x[3]
...
class CompressionSpringProblem(Engineer):
    ...
    def get_cons(self, x):
        ...
        g2 = (4 * x[1] ** 2 - x[0] * x[1]) / (12566 * (x[2] * x[0] ** 3 - x[0] ** 4)) + 1.0 / (5108 * x[0] ** 2) - 1
...
class CantileverBeamProblem(Engineer):
    ...
    def get_cons(self, x):
        """Compute the constraint function values for the candidate solution."""
        g1 = 61.0 / x[0] ** 3 + 37.0 / x[1] ** 3 + 19.0 / x[2] ** 3 + 7.0 / x[3] ** 3 + 1.0 / x[4] ** 3 - 1
        return np.array(
            [
                g1,
            ]
...
class IBeamProblem(Engineer):
    ...
    def __init__(self, f_penalty=None):
        ...
        self._n_objs = 1
        self._n_cons = 2
        self._bounds = [(10, 50.0), (10, 80.0), (0.9, 5.0), (0.9, 5.0)]
        self.check_penalty_func(f_penalty)

    def get_objs(self, x):
        """Compute the objective function values for the candidate solution."""
        f1 = 500.0 / ((x[2] * (x[1] - 2 * x[3]) ** 3) / 12 + (x[0] * x[3] ** 3 / 6) + 2 * x[0] * x[3] * (x[1] - x[3]) ** 2)
        return np.array([f1])
    ...
    def get_cons(self, x):
        ...
        g2 = (
            (18 * x[1] * 10**4) / (x[2] * (x[1] - 2 * x[3]) ** 3 + 2 * x[0] * x[2] * (4 * x[3] ** 2 + 3 * x[1] * (x[1] - 2 * x[3])))
            + 15 * x[0] * 10**3 / ((x[1] - 2 * x[3]) * x[2] ** 2 + 2 * x[2] * x[0] ** 3)
            - 56
        )
...
class PistonLeverProblem(Engineer):
    ...
    def __init__(self, f_penalty=None):
        ...
        self._n_objs = 1
        self._n_cons = 4
        self._bounds = [(0.05, 500), (0.05, 500.0), (0.05, 120.0), (0.05, 500.0)]
        self.L = 240
        self.M_max = 1.8 * 10**6
        self.P = 1500
        self.Q = 10000
...
class CorrugatedBulkheadProblem(Engineer):
    ...
    def __init__(self, f_penalty=None):
        ...
        self._n_objs = 1
        self._n_cons = 6
        self._bounds = [(0.0, 100), (0.0, 100.0), (0.0, 100.0), (0.0, 5.0)]
        self.check_penalty_func(f_penalty)
    ...
    def get_cons(self, x):
        ...
        g2 = -x[3] * x[1] ** 2 * (0.2 * x[0] + x[2] / 12) + 2.2 * (8.94 * (x[0] + np.sqrt(np.abs(x[2] ** 2 - x[1] ** 2)))) ** (4.0 / 3)
...
class ReinforcedConcreateBeamProblem(Engineer):
    ...
    def __init__(self, f_penalty=None):
        ...
        self.le.fit(self.x0)
        self._bounds = [(0.0, 10.99), (28.0, 40.99), (5.0, 10.0)]
        self.check_penalty_func(f_penalty)
