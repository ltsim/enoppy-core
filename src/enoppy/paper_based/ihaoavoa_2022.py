...
# --------------------------------------------------%
"""Engineering problems from the IHAOAVOA 2022 paper."""
...
class TensionCompressionSpringProblem(Engineer):
    ...
    def get_cons(self, x):
        ...
        g2 = (4 * x[1] ** 2 - x[0] * x[1]) / (12566 * (x[1] * x[0] ** 3 - x[0] ** 4)) + 1.0 / (5108 * x[0] ** 2)
...
class SpeedReducerProblem(Engineer):
    ...
    def get_objs(self, x):
        """Compute the objective function values for the candidate solution."""
        f1 = (
            0.7584 * x[0] * x[1] ** 2 * (3.3333 * x[2] ** 2 + 14.9334 * x[2] - 43.0934)
...
class RollingElementBearingProblem(Engineer):
    ...
    def __init__(self, f_penalty=None):
        ...
        self._bounds = [(125.0, 150.0), (10.5, 31.5), (4, 50), (0.515, 0.6), (0.515, 0.6), (0.4, 0.5), (0.6, 0.7), (0.3, 0.4), (0.02, 0.1), (0.6, 0.85)]
        self.check_penalty_func(f_penalty)
    ...
    def get_objs(self, x):
        """Compute the objective function values for the candidate solution."""
        gama = x[1] / x[0]
        t1 = 37.91 * (1 + (1.04 * ((1 - gama) / (1 + gama)) ** 1.72 * (x[3] / x[4] * (2 * x[4] - 1) / (2 * x[3] - 1)) ** 0.41) ** (10.0 / 3)) ** (-0.3)
        fc = t1 * (gama**0.3 * (1 - gama) ** 1.39 / ((1 + gama) ** (1.0 / 3))) * (2 * x[3] / (2 * x[3] - 1)) ** 0.41
        if x[1] <= 25.4:
            f1 = fc * x[2] ** (2.0 / 3) * x[1] ** 1.8
        else:
            f1 = 3.647 * fc * x[2] ** (2.0 / 3) * x[1] ** 1.4
        return np.array(
            [
                f1,
            ]
    ...
    def get_cons(self, x):
        ...
        theta0 = 2 * np.pi - 1.0 / np.cos(xx / yy)

        g1 = theta0 / (2.0 / np.sin(x[1] / x[0])) - x[2] + 1
