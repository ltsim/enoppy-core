...
class WeldedBeamProblem(Engineer):
    ...
    def get_cons(self, x):
        Pc = 4.013 * self.E * np.sqrt(x[2] ** 2 * x[3] ** 6 / 36) / self.L ** 2 * (
...
class SpeedReducerProblem(Engineer):
    ...
    def get_objs(self, x):
        f1 = 0.7584 * x[0] * x[1] ** 2 * (3.3333 * x[2] ** 2 + 14.9334 * x[2] - 43.0934) - 1.508 * x[0] * (
...
class RollingElementBearingProblem(Engineer):
    ...
    def __init__(self, f_penalty=None):
        ...
        self._bounds = [(125., 150.), (10.5, 31.5), (4, 50), (0.515, 0.6), (0.515, 0.6), (0.4, 0.5), (0.6, 0.7),
                        (0.3, 0.4), (0.02, 0.1), (0.6, 0.85)]
        self.check_penalty_func(f_penalty)
    ...
    def get_objs(self, x):
        gama = x[1] / x[0]
        t1 = 37.91 * (1 + (1.04 * ((1 - gama) / (1 + gama)) ** 1.72 * (
                    x[3] / x[4] * (2 * x[4] - 1) / (2 * x[3] - 1)) ** 0.41) ** (10. / 3)) ** (-0.3)
